...
local id = args.id
if not t or t == "" then
  local v = ui.form("菜谱大全", {
    { type = "select", id = "type", label = "查询类型", options = { "rank", "search", "detail" }, default = "search" },
    { type = "input", id = "keyword", label = "关键词（search）", default = args.keyword or "" },
    { type = "input", id = "id", label = "菜谱 ID（detail）", default = args.id or "" }
  })
  if not v then return { cancelled = true } end
  t, keyword, id = v.type, v.keyword, v.id
end
t = t or "rank"
if t == "search" and (not keyword or keyword == "") then
  return { error = "搜索模式需要提供关键词 keyword" }
end
if t == "detail" and (not id or id == "") then
  return { error = "详情模式需要提供菜谱 ID" }
end

local query = "type=" .. urlencode(t)
if args.rankType and args.rankType ~= "" then query = query .. "&rankType=" .. urlencode(args.rankType) end
if keyword and keyword ~= "" then query = query .. "&keyword=" .. urlencode(keyword) end
if id and id ~= "" then query = query .. "&id=" .. urlencode(id) end
if args.page and tostring(args.page) ~= "" then query = query .. "&page=" .. urlencode(tostring(args.page)) end

local key = api_key()
local res = tool.httpGet("https://zenneko.top/api/caipu.php?" .. query, {
  ["Authorization"] = "Bearer " .. key,
  ["X-Open-Key"] = key
})
local err = check(res)
if err then return { error = err } end
return { type = t, response = res }
