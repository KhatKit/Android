...
- 新增火山引擎 TTS（豆包语音，SSE 流式合成，凭据使用 `X-Api-Key`）
- 输入栏背景效果支持 `模糊 / 玻璃` 两种类型（Haze 升级至 2.0.0-rc01）
- JavaScript 引擎迁移到 `quickjs-kt` 并修复资源泄露（Lua 仍为 Rust 单引擎）
- 刷新 MiniMax 地区与 TTS 默认值
- 修复部分非聊天请求缺少 session id 头的问题
- 完善所有语言的重新生成消息确认提示
- 修复内联代码连字显示问题、其他依赖升级
